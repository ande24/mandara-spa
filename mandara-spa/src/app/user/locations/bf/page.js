...
export default function Page() {
  ...
  useEffect (() => {
    const fetchBranch = async() => {
      const branchSnap = await getDoc(doc(db, "branches", "enGNgyK94mXKZj4NNGD6"));
        ...
        <div className="flex flex-col justify-center items-center w-full mx-auto p-35">
          ...
          <div className="mt-4 w-2xl bg-gray-200 text-gray-800 p-4 text-sm rounded-lg flex transition-all hover:bg-gray-50 hover:scale-105 hover:shadow-md shadow-black md:flex-row md:items-center justify-between">
            <div className="space-y-2 font-serif w-sm">
              ...
              <button onClick={handleBooking} className="bg-[#502424] text-xs w-1/3 text-white font-semibold p-2 h-full rounded-lg hover:bg-[#e0d8ad] hover:scale-105 transition-all md:ml-6">
                Book Now
              </button>
          </div>
        </div>
