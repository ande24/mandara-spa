...
const ManageService = ({ onClose }) => {
    ...
    useEffect(() => {
        if (userData && userData.branch_id) {
            ...
            const unsubscribeBranch = onSnapshot(branchRef, (docSnap) => {
                if (docSnap.exists()) {
    ...
    useEffect(() => {
        if (userData?.branch_id) {
            ...
            const unsubscribeServices = onSnapshot(serviceCollection, (serviceSnapshot) => {
    ...
    useEffect(() => {
        if (selectedService && userData?.branch_id) {
            const branchRef = doc(db, "branches", userData.branch_id);
            const serviceRef = doc(branchRef, "services", selectedService);
            const unsubscribeService = onSnapshot(serviceRef, (docSnap) => {
                if (docSnap.exists()) {
